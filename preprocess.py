...
def load_data(dataset):
	...
	if dataset == 'synthetic':
		...
		labels = pd.read_excel(os.path.join(dataset_folder, 'custom_labels.xlsx'))
		train = pd.read_excel(os.path.join(dataset_folder, 'custom_train.xlsx'))
		test = pd.read_excel(os.path.join(dataset_folder, 'custom_test.xlsx'))
		# print(len(labels))
		# print(len(train))
		# print(len(test))
		...
		labels = labels.values[:,1].astype(int).reshape(-1,1)
		...
		# print(train.shape)
		print(labels[:,:])
		np.savetxt("labels_mba.txt",labels)
		for file in ['train', 'test', 'labels']:
			np.save(os.path.join(folder, f'{file}.npy'), eval(file))
	else:
